// The contents of this file were directly inspired by @author Shashank Tiwari and his article "Build a Realtime group chat app in Golang using WebSockets"
...
func handleSocketPayloadEvents(client *Client, socketEventPayload SocketEventStruct) {
	...
	case "message":
		...
		// ADAM/DARRION, Here, I think that each message should be saved between the two parties so that it can later be stored in a database
...
func getUsernameByUserID(hub *Hub, userID string) string {
	...
	// Instead of connecting users to only those currently using websockets, see if possible to link all existing connections and send messages there
	return username
}
